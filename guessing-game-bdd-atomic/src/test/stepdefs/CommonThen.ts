...
export class CommonThen {
    ...
    public async thenTheRequestFailsDueTo(reason: string) {
        const response = this.scenarioContext.getLastResponse();
        const statusCode = response.status;
        assert(statusCode >= 400 && statusCode < 500,
    ...
    public async thenRequestSucceeds() {
        const response = this.scenarioContext.getLastResponse();
        const statusCode = response.status;
    ...
    @Then('response:')
    public async thenResponse(dataTable: DataTable) {
        const response = this.scenarioContext.getLastResponse();
        const statusCode = response.status;
        assert(statusCode >= 200 && statusCode < 300,
